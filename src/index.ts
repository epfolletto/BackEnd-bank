...
app.post("/pay", (req: Request, res: Response) => {
  try {
    ...
    const actualDay = new Date().getDate() > 9 ? new Date().getDate() : `0${new Date().getDate()}`;
    const actualMonth = new Date().getMonth()+1 > 9 ? new Date().getMonth()+1 : `0${new Date().getMonth()+1}`;
